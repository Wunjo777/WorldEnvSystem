    ...
    SubShader
    {
        ...
        HLSLINCLUDE 
            ...
            float SampleDensity(float3 p)
            {
                ...
                float2 uv = p.xz / float2(size.x, size.z);//存疑？？？？？？？？？？？？？？？
            ...
            float2 CloudRayMarching(float3 stPoint, float3 dir, float dstLimit, float blueNoise)
            {
                ...
                float curStep = 0;//blueNoise*300;
                float totalEnergy = 0;
                float3 curPos = stPoint;

                //Henyey-Greenstein Phase Function for scatter forward(silver lining effect) 
                float phaseVal = HenyeyGreenstein(_MainLightPosition,dir,0.15);//g = 0.15
            ...
            float4 frag (Varings IN) : SV_Target
            {
                ...
                float blueNoise = tex2D(_BlueNoise, TRANSFORM_TEX(IN.uv, _BlueNoise)*4);
                ...
                float4 col = tex2D(_MainTex, TRANSFORM_TEX(IN.uv, _MainTex));
